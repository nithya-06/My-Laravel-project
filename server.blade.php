...
//add to hireme
	...
	if (empty($desc3)) { array_push($errors, "Description3 is required"); }
		...
		// register user if there are no errors in the form
		if (count($errors) == 0) {
			$query = "INSERT INTO hire info (price,sub,desc0,desc1,desc2,desc3)
...
//add to work
	if (isset($_POST['add_work'])) {
		...
		// register user if there are no errors in the form
		if (count($errors) == 0) {
			$query = "INSERT INTO latest works (WORK_TITLE,WORK _DESC)
			VALUES('$worktitle', '$workdescription')";
			mysqli_query($db, $query);
...
//delete from education
	...
	if (isset($_POST['delete_education'])) {
		...
		if (empty($degree)) { array_push($errors, "Type the degree to be deleted"); }
		if (empty($university)) { array_push($errors, "Type the university to be deleted"); }
		if (empty($startdate)) { array_push($errors, "Type the startdate to be deleted"); }
		if (empty($enddate)) { array_push($errors, "Type the End date to be deleted"); }


		// register user if there are no errors in the form
		if (count($errors) == 0) {
			$query = "DELETE * FROM users WHERE major=$major";
			mysqli_query($db, $query);
...
///delete from work experience
	...
	if (isset($_POST['delete_workexperience'])) {
		...
		// form validation: ensure that the form is correctly filled
		if (empty($jobtitle)) { array_push($errors, "Type the Job title to be deleted"); }
		if (empty($workdescription)) { array_push($errors, "Type theworkdescription to be deleted"); }
		if (empty($jobposition)) { array_push($errors, "Type the Job position to be deleted"); }
		...
		// register user if there are no errors in the form
		if (count($errors) == 0) {
			$query = "DELETE * FROM users WHERE jobtitle=$jobtitle";
			mysqli_query($db, $query);
...
//delete from  hireme
	...
	if (empty($desc3)) { array_push($errors, "Description3 is required"); }
		...
		// register user if there are no errors in the form
		if (count($errors) == 0) {
			$query = "DELETE * FROM hire info WHERE price=$price";
...
//delete from work
	if (isset($_POST['delete_work'])) {
		...
		// register user if there are no errors in the form
		if (count($errors) == 0) {
			$query = "DELETE * FROM latest works WHERE WORK_TITLE=$worktitle";
			mysqli_query($db, $query);
